		...
		if ( !strcmp(cp1,"LogFile") ) {
		   ...
		   strcpy(logfile,cp2);
			...
			while ( fgets(buf,MAX_CFG_LINE_LENGTH,inp) ) {
		     ...
		     printf("Error in config file ../config/cyusb.conf: %s \n",buf);
		     exit(1);
		}
	}
...
int cyusb_open(void)
{
	...
	fd1 = open("../config/cyusb.conf", O_RDONLY);
	if ( fd1 < 0 ) {
	   printf("../config/cyusb.conf file not found. Exiting\n");
	   return -1;
	}
	else {
	   close(fd1);
